...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // MMOCoin: starting difficulty is 1 / 2^12
...
{
    // MMOCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespanNEW = 60 ; // MMOCoin: every 1 minute
static const int64 nTargetSpacing = 60; // MMOCoin: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // MMOCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
{
    // MMOCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Court backs 'right to be forgotten'";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MMOCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MMOCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MMOCoinMiner(CWallet *pwallet)
{
    printf("MMOCoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running MMOCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MMOCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MMOCoinMiner, pwallet));
}
